#include "Geometric Primitive.hlsli"

VS_OUT main(float4 position : POSITION, float4 normal : NORMAL)	// cpp側のセマンティクスと同名にすることでその値を使用できる
{
    VS_OUT vout;
    vout.poosition = mul(position, mul(world, view_projection));    // world * n = ワールドになる
    normal.w = 0;
    float4 Normal = normalize(mul(normal, world));       // n * world = ローカルになる
