...
Static_Mesh::Static_Mesh(const wchar_t* obj_filename, const char* vs_cso_name, const char* ps_cso_name) {
	...
	// OBJファイルの読み込み
	{
		...
		while (fin)
		{
			...
			// 法線読み込み
			else if (0 == wcscmp(command, L"vn"))	// "vn"をファイル内に見つけたら
			{
				FLOAT i, j, k;
				fin >> i >> j >> k;
	...
	for (auto& material : materials)
	{
		for (int i = 0; i < material.TEXTURE_NUM; i++) {	// bmpmapに対応だいぶ無理やり作成 テクスチャ(SRV)枚数分回す もっといい方法ないんかな？
			if (0 == wcscmp(material.texture_filenames[i].c_str(), L"")) {	// texture_filenamesとL""(テクスチャなし)を比較、同じ(return 0)であればダミーテクスチャを作成
				make_dummy_texture(&material.shader_resource_view[i], 0xFFFFFFFF, 16);	// 0xFFFFFFFF:テクスチャ色指定 白
				//make_dummy_texture(device, &material.shader_resource_view[1], 0xFFFF7F7F, 16);	// 0xFFFF7F7F:法線マップ用の色指定 青
				break;
			}
			else {	// 通常通りテクスチャをロード
				load_texture_from_file(material.texture_filenames[i].c_str(), &material.shader_resource_view[i], &texture2d_desc);
			}
		}
	}
